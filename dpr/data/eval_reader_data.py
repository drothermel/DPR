...
def get_best_spans(
    ...
    """
    # Note: only need evcf if we're constraining sids in here
    # Get the token ranges
    qid_cid = passage_id.split('__dev_')
    cid = qid_cid[1]
    cid_data = cid_range_dict[cid]
    tok_ranges = [(a[0]-1, a[1]-1) for a in cid_data['token_inds']]

    scores = []
    for tind, tr in enumerate(tok_ranges):
        range_start = tr[0]
        range_end = tr[1]
        if False:
            # This allows for any subset of the string to be chosen
            for st_si in range(range_start, range_end):
                max_st_end = min(range_end, st_si + max_answer_length)
                for st_ei in range(st_si, max_st_end):
                    start_s = start_logits[st_si]
                    end_s = end_logits[st_ei]
                    scores.append(((st_si, st_ei), start_s + end_s))
        else:
            st_si = range_start
            st_ei = range_end - 1
            if st_si >= len(start_logits) or st_ei >= len(end_logits):
                logger.info(f">> ERROR: {passage_id} {tr} start or end out of range, {st_si} {st_ei} {len(start_logits)} {len(end_logits)}")
                continue
            start_s = start_logits[st_si]
            end_s = end_logits[st_ei]
            scores.append(((st_si, st_ei), start_s + end_s, tind))

    # Original method
    #for (i, s) in enumerate(start_logits):
    #    for (j, e) in enumerate(end_logits[i : i + max_answer_length]):
    #        scores.append(((i, i + j), s + e))
    #        # I think this will let me see the answer span considered
    #        #predicted_answer = tensorizer.to_string(ctx_ids[i : i+j + 1])
    ...
    for (start_index, end_index), score, tind in scores:
        if start_index > end_index:
            logger.info(f">> ERROR: start index {start_index} > end index {end_index} for {passage_id}")
            continue
